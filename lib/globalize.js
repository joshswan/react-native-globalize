...
export default class {
   ...
   */
  getUnitFormatter(unit, options) {
    const key = cacheKey(options);

    if (!this._relativeTimeFormatters[unit]) {
      this._unitFormatters[unit] = {};
    }

    if (!this._unitFormatters[unit][key]) {
      this._unitFormatters[unit][key] = this.globalize.unitFormatter(unit, options);
    }

    return this._unitFormatters[unit][key];
  }
}
