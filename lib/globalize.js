...
function findFallbackLocale(locale) {
  ...
  var localesCount = locales.length;

  for (var i = locale.length - 1; i > 1; i -= 1) {
    var key = locale.substring(0, i);

    for (var n = 0; n < localesCount; n += 1) {
...
function _localeIsLoaded(locale) {
...
var _class = function () {
  ...
  _createClass(_class, [{
    ...
    key: 'localeIsLoaded',
    value: function localeIsLoaded(locale) {
      return _localeIsLoaded(locale);
    }
  }]);
