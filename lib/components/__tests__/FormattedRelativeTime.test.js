...
describe('<FormattedRelativeTime />', () => {
  ...
  describe('unit', () => {
    test('supports best unit option', () => {
      const tree = createComponentWithGlobalize(<FormattedRelativeTime value={new Date(2015, 0, 1)} unit="best" />).toJSON();
  ...
  describe('value', () => {
    test('supports date instances for value', () => {
      const tree = createComponentWithGlobalize(<FormattedRelativeTime value={new Date(2017, 0, 1)} unit="day" />).toJSON();
